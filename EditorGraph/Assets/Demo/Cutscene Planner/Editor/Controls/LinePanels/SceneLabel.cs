...
{
    /// <summary> Represents the scene label</summary>
    public class SceneLabel : Label
    {
        /// <summary> Dimensions of the label on the texture. </summary>
        public override Rect TextureRect  { get { return new Rect(_timeLinePanel.TimeToWorldX(SourceData.Start), Owner.Rect.y, _timeLinePanel.TimeLineEngine.TimeToX(SourceData.Duration), Owner.Rect.height); } }
        ...
        public override void Draw(Texture2D oryginal, Texture2D toDraw)
        {
            ...
            int maxHeight = (int)Owner.Rect.height - Owner.BorderThickness * 2;


            Rect rect = TextureRect;

            int x = (int)rect.x;
            int y = Owner.BorderThickness;
